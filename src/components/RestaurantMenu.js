    ...
    <div className="flex flex-wrap m-4">
      <div className="p-3">
        ...
        <h1 className="font-bold text-3xl py-3 text-center">
          {restaurantApi?.cards[2]?.card?.card?.info?.name}
        </h1>
        <img
          className="w-96 py-2"
          src={
            IMG_CDN_URL +
            restaurantApi?.cards[2]?.card?.card?.info?.cloudinaryImageId
          }
        />
        <h3 className="px-4 font-semibold text-lg">
          Area: {restaurantApi?.cards[2]?.card?.card?.info?.areaName}
        </h3>
        <h3 className="px-4 font-semibold text-lg">
          City: {restaurantApi?.cards[2]?.card?.card?.info?.city}
        </h3>
        <h3 className="px-4 font-semibold text-lg">
          Rating: {restaurantApi?.cards[2]?.card?.card?.info?.avgRating} stars
        </h3>
        <h3 className="px-4 font-semibold text-lg">
          {restaurantApi?.cards[2]?.card?.card?.info?.costForTwo / 100 +
            "/- for two"}
        </h3>
      </div>
      <div className="px-10 py-3">
        <h1 className="font-bold text-3xl py-3" data-testid="menu">
          Menu
        </h1>
        <ul>
          {Object.values(
            restaurant[0]?.data?.cards[2]?.groupedCard?.cardGroupMap?.REGULAR
              ?.cards[2]?.card?.card?.itemCards
          ).map((item) => (
            <li className="py-1" key={item?.card?.info?.id}>
              {item?.card?.info?.name} -{" "}
              <button
                data-testid="add-btn"
                onClick={() => addFoodItem(item?.card?.info)}
                className="rounded-md p-1 bg-green-200"
              >
                Add
              </button>{" "}
              -{" "}
              <button
                className="p-1 rounded-md bg-red-200"
                onClick={() => removeItems(item?.card?.info?.id)}
              >
                Remove
              </button>
            </li>
